...
2.	Разметка разломов идет вручную.
2.	Информация долго обрабатывается.
Причин несколько:
1.	Очень много данных нужно обработать. Собираются данные с сейсмографов на поверхности. Анализируется много информации, так как только одна скважина стоит миллион рублей.
Даже когда пробурина скважина нужно дальше исследовать на глубине и до определенного расстояния земную кору. В итоге получают очень много данных, обрабатывают и в итоге получается объемное изображение. Изучив данные, представленные в виде объемного куба, можно дальше расширять область добычи ресурсов.
2.	Анализ данных проходит часто вручную и потому занимает много времени.

* Этот проект позволяет уменьшить время разметки разломов, которое до этого происходило в основном вручную. К примеру, целые институты работают с данными с месторождения. Мы же уменьшаем меньшаем время работы на порядок (несколько лет - несколько месяцев) и количество людей занятых рутинной разметкой (сотни - десятки).

*Несколько предложений о задаче и решении: какую задачу проект решает, какие решения есть и чем это лучше.

*Как можно будет воспользоваться результатом? Например, проект — это сервис на сайте / предварительно обученная нейросеть, зашитая в приложение / мобильное приложение / HTTP API / …
